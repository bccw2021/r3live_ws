                      ...
                      ImageArr &m_images, PointCloud &m_pointcloud )
{
    ...
    
    {
        ...
        printf( "Delaunay tetrahedralization completed: %lu points -> %lu vertices, %lu (+%lu) cells, %lu (+%lu) faces (%s)\r\n", (unsigned long)indices.size(),
                (unsigned long)delaunay.number_of_vertices(), (unsigned long)delaunay.number_of_finite_cells(), (unsigned long)(delaunay.number_of_cells() - delaunay.number_of_finite_cells()),
                (unsigned long)delaunay.number_of_finite_facets(), (unsigned long)(delaunay.number_of_facets() - delaunay.number_of_finite_facets()), TD_TIMER_GET_FMT().c_str() );
    }
...
#pragma omp atomic
        ...
        // TD_TIMER_GET_FMT().c_str() );
        printf( "Delaunay tetrahedras weighting completed: %lu cells, %lu faces (%s)\r\n", (unsigned long)delaunay.number_of_cells(), (unsigned long)delaunay.number_of_facets(),
                TD_TIMER_GET_FMT().c_str() );
    }
